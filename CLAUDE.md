...
- **目前規模**：約 268 個任務節點，含 9 大分類（編成/出撃/演習/遠征/補給入渠/工廠/改装/改修/ケッコンカッコカリ）+ 多維標籤系統
...
- **同名任務**：同一 `name` 可有多個任務（如 t201/t202 都叫「潜水艦隊」出撃せよ！），用 `content` 欄位區分（基礎任務 vs 月刊任務）。UI 透過 id 唯一性區分
