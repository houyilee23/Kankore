# Reads data/improvements.json and inlines it into guide.html as `const IMPROVEMENTS = {...};`.
# Writes between sentinels // <IMPROVEMENTS_START> ... // <IMPROVEMENTS_END>.
# On first run (no sentinels yet), inserts the block immediately after // <TASKS_END>.
# guide.html is written as UTF-8 without BOM.
#
# Run with: powershell -NoProfile -ExecutionPolicy Bypass -File scripts/sync_improvements.ps1

$ErrorActionPreference = 'Stop'

$root = Split-Path -Parent $PSScriptRoot
$htmlPath = Join-Path $root 'guide.html'
$jsonPath = Join-Path $root 'data\improvements.json'

# Same unescaper as sync_tasks.ps1.
...
# Read improvements data.
$dataText = [System.IO.File]::ReadAllText($jsonPath, [System.Text.Encoding]::UTF8)
$data = $dataText | ConvertFrom-Json

if (-not $data.items) {
    throw "improvements.json missing 'items' array"
}

$itemFieldOrder = @('category', 'name', 'secretary', 'required_equipment', 'updates_to', 'notes')
...
$startMark = '// <IMPROVEMENTS_START> generated from data/improvements.json by scripts/sync_improvements.ps1; do not edit by hand'
$endMark   = '// <IMPROVEMENTS_END>'

$blockLines.Add($startMark)
$blockLines.Add('const IMPROVEMENTS = {')
$blockLines.Add('  "version": ' + $data.version + ',')
$blockLines.Add('  "source": ' + (CompactValue $data.source) + ',')
$blockLines.Add('  "fetched_at": ' + (CompactValue $data.fetched_at) + ',')
...
$sentinelPattern = '(?s)// <IMPROVEMENTS_START>.*?// <IMPROVEMENTS_END>'
...
if ([regex]::IsMatch($html, $sentinelPattern)) {
    ...
    # First run: insert immediately after // <TASKS_END> (with one blank line separator).
    $tasksEndPattern = '// <TASKS_END>'
    if (-not [regex]::IsMatch($html, $tasksEndPattern)) {
        throw "Could not locate '// <TASKS_END>' anchor in $htmlPath. Run sync_tasks.ps1 first to bootstrap TASKS sentinels."
    }
    $insertText = "// <TASKS_END>`n`n" + $newBlock
    $insertEvaluator = [System.Text.RegularExpressions.MatchEvaluator] { param($m) $insertText }
    $regex = New-Object System.Text.RegularExpressions.Regex($tasksEndPattern)
    $newHtml = $regex.Replace($html, $insertEvaluator, 1)
    $mode = 'bootstrap (inserted after TASKS_END)'
}

$encoding = New-Object System.Text.UTF8Encoding($false)
[System.IO.File]::WriteAllText($htmlPath, $newHtml, $encoding)

Write-Host "Synced $($items.Count) improvement entries to guide.html ($mode)"
